<%-- 
    Document   : cart
    Created on : Mar 11, 2023, 8:41:06 PM
    Author     : DELL
--%>
...
<!--        <h3>${requestScope.error}</h3>-->
        <c:set var="c" value="${requestScope.product}"></c:set>
            ...
            ID (Category): <input type="text" readonly name="id" value="${c.id}"/></br>
            Price: <input type="number" name="price" value="${c.price}"/></br>
            Name: <input type="text" name="name" value="${c.name}"/></br>
            Description: <textarea name="description">${c.description}</textarea></br><!--id="editor" -->
            Image: <input type="text" name="image" value="${c.image}"/></br>  
            ID (CateID): <input type="text" readonly name="cid" value="${c.cateID}"/></br>
